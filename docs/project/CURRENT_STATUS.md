...
現在は **Sprint 0.8.1 / Issue #21「Page operation selection stability」** の実装レビュー段階。
...
Sprint 0.8.1 では、削除・複製・Undo / Redo 後に選択状態と表示位置が自然に残るようにする。
...
0.8.x は、v1.0 に向けて「操作後にユーザーが迷わないこと」を固める段階。削除後に選択が完全に消えるより、近いページを選択したままにする方が、次操作へつながりやすく、毎日使うツールとして自然。
