...
現在は **Sprint 0.8.2 / Issue #22「Page Map drag reorder」** の実装レビュー段階。
...
Sprint 0.8.2 では、Page Map からもページをドラッグ並び替えできるようにし、Page Viewer、選択状態、Undo / Redo と同期させる。
...
0.8.x は、v1.0 に向けて「操作後にユーザーが迷わないこと」と「Page Map と Page Viewer の一貫性」を固める段階。Page Map は単なる一覧ではなく、ページ構成を直接操作できる地図として育てる。
